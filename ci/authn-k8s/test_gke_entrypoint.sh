...
# ENDOFLINE
# )
export LOCAL_DEV_VOLUME='emptyDir: {}'
...
function finish {
...

  {
    if [[ "$pod_name" != "" ]]; then
      ...
      kubectl cp "$pod_name:/opt/conjur-server/coverage/.resultset.json" \
  ...
  echo '-----'
  kubectl --ignore-not-found=true delete namespace \
    "$CONJUR_AUTHN_K8S_TEST_NAMESPACE"
...
function createNamespace() {
  ...
  old_namespaces=$(kubectl get namespaces | \
    awk '$1 ~ /test-/ && $3 !~ /[m|s]/ { print $1; }')
  [ ! -z "${old_namespaces}" ] && \
  ...
  kubectl create --filename dev/dev_conjur_authenticator_role_binding.${TEMPLATE_TAG}yaml
}
...
function launchConjurMaster() {
  ...
  sed -e "s#{{ CONJUR_AUTHN_K8S_TAG }}#$CONJUR_AUTHN_K8S_TAG#g" \
  ...
  conjur_pod=$(kubectl get pods \
    --selector app=conjur-authn-k8s \
    --output=jsonpath='{.items[].metadata.name}')

  # TODO: Refactor this -- passing bash code to be eval'd by a function
  #       isn't good practice.
  wait_for_it 300 "kubectl describe po $conjur_pod | \
    grep Status: | grep -q Running"

  # wait for the 'conjurctl server' entrypoint to finish
  kubectl exec "$conjur_pod" -- bash -c "while ! curl -sI localhost:80 > \
    /dev/null; do sleep 1; done"
  
  API_KEY=$(kubectl exec "$conjur_pod" -- \
    conjurctl account create cucumber | \
    tail -n 1 | \
    awk '{ print $NF }')
  export API_KEY
}
...
function copyNginxSSLCert() {
  nginx_pod=$(kubectl get pods --selector app=nginx-authn-k8s \
    --output=jsonpath='{.items[].metadata.name}')
  cucumber_pod=$(kubectl get pods --selector app=cucumber-authn-k8s \
    --output=jsonpath='{.items[].metadata.name}')
...
function copyConjurPolicies() {
  cli_pod=$(kubectl get pod --selector app=conjur-cli --no-headers | \
    grep Running | \
    awk '{ print $1 }')
...
function loadConjurPolicies() {
  ...
  # load policies
  kubectl exec "$cli_pod" -- \
    conjur policy load root "/policies/policy.${TEMPLATE_TAG}yml"

  # init ca certs
  conjur_pod=$(kubectl get pod --selector app=conjur-authn-k8s --no-headers | \
...
function launchInventoryServices() {
  ...
  wait_for_it 300 "kubectl describe po inventory | \
    grep Status: | grep -c Running | grep -q 4"
}
